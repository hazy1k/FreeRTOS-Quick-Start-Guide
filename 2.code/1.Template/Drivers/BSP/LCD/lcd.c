...
void lcd_init(void)
{
    ...
    /* 初始化完成以后,提速 */
    if (lcddev.id == 0X7789 || lcddev.id == 0x7796 || lcddev.id == 0x9806)            /* ST7789/ST7796/ILI9806 提速 */
    {
        /* 重新配置写时序控制寄存器的时序 */
    ...
    else if (lcddev.id == 0X9341 || lcddev.id == 0X1963 || lcddev.id == 0x5310 || lcddev.id == 0x5510)  /* 如果是这几个IC,则设置WR时序为最快 */
    {
        /* 重新配置写时序控制寄存器的时序 */
